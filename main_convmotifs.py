...
def main():
    ...
    model = EGNNC(input_dim, hidden_dim, output_dim,
                    channel_dim, config['dropout'], config['device'], config['num_layers'])
    ...
    if config["classifier"] == "cnn":
        cnn = models.TriangularMotifsCNN(num_channels = 4, activation=config['activation'], dropout=config['dropout'], device=config['device'])
